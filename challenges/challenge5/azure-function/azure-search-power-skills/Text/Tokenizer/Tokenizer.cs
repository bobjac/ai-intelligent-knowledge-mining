...
namespace AzureCognitiveSearch.PowerSkills.Text.Tokenizer
{
    public static class Tokenizer
    {
            ...
            WebApiSkillResponse response = WebApiSkillHelpers.ProcessRequestRecords(skillName, requestRecords,
                (inRecord, outRecord) =>
                {
                 ...
                 //   outRecord.Data["words"] = predictionEngine.Predict(text).Words ?? Array.Empty<string>();
                    var transformedTextData = predictionEngine.Predict(text).Words ?? Array.Empty<string>();
                    outRecord.Data["words"] = GetTopTenWords(transformedTextData);
        ...
        private static string[] GetTopTenWords(string[] inputWords)
        {
            var keywords = inputWords.GroupBy(x => x).OrderByDescending(x => x.Count());
